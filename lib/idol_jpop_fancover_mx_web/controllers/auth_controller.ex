defmodule IdolJpopFancoverMxWeb.AuthController do
  ...
  alias IdolJpopFancoverMxWeb.UserFromAuth
  ...
  def callback(%Plug.Conn{assigns: %{ueberauth_auth: auth}} = conn, _params, current_user, _claims) do
    case UserFromAuth.get_or_insert(auth, current_user, Repo) do
      {:ok, user} ->
        conn
        |> put_flash(:info, "Signed in as #{user.nombre}")
  ...
  def credentials(conn, _params, current_user, {:ok, _claims}) do
    ...
    user = %{nombre: current_user.nombre, email: current_user.email, id: current_user.id}
  ...
  defp auths(nil), do: []
  defp auths(%IdolJpopFancoverMxWeb.Usuario{} = user) do
    assoc(user, :autorizaciones)
      |> Repo.all
      |> Enum.map(&(&1.provider))
  end
end
defmodule IdolJpopFancoverMxWeb.UserFromAuth do
  alias IdolJpopFancoverMxWeb.Usuario
  alias IdolJpopFancoverMxWeb.Autorizacion
  ...
  defp user_from_authorization(authorization, current_user, repo) do
    case repo.one(Ecto.assoc(authorization, :usuario)) do
  ...
  defp auth_and_validate(%{provider: :identity} = auth, repo) do
    case repo.get_by(Autorizacion, uid: uid_from_auth(auth), provider: to_string(auth.provider)) do
      nil -> {:error, :not_found}
      authorization ->
  ...
  defp authorization_from_auth(user, auth, repo) do
    authorization = Ecto.build_assoc(user, :autorizaciones)
    result = Autorizacion.changeset(
      authorization,
      scrub(
        %{
